...
class EffUNet(nn.Module):
    def __init__(
        ...
        # self.encoder = EfficientNet3D.from_name(
        #     "efficientnet-b4",
        #     override_params={'num_classes': 256},
        #     in_channels=1, 
        #     num_pools=num_pools,
        #     )
...
#             factor_e=factor,
            ...
            # factor_d=[48,24,32,56,160,448], # efficient-b4 feature maps
#             factor_d=[get_outfmaps(l) for l in self.pyramid], # efficient-b4 feature maps
            use_deep=use_deep,
            )
    ...
    def forward(self, x): # x is an image
        self.encoder(x)
        # print([s.shape for s in self.down])
